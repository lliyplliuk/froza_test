...
class Connection extends DBALConnection
{
    ...
    public function insertIgnore($tableExpression, array $data, array $types = [])
    {
        if (empty($data)) {
            return $this->executeStatement('INSERT IGNORE INTO ' . $tableExpression . ' ()' . ' VALUES ()');
        }

        return $this->isArrayMultidimensional($data) ?
            $this->insertIgnoreMultiple($tableExpression, $data, $types) :
            parent::insert($tableExpression, $data, $types);
    }
     ...
     * @param array $data An associative array (multidimensional, if insertion/update involves multiple records) containing column-value pairs.
     * @param array $types Types of the merged $data and $identifier arrays in that order.
     * @param array $columnsToUpdate Columns to be updated in case of duplicates.
     *
     * @return int The number of affected rows.
     */
    public function upsert($tableExpression, array $data, array $types = [], array $columnsToUpdate = [], array $valuesToUpdate = [])
    {
        if (!$this->isArrayMultidimensional($data)) {
            $data = [$data];
        }

        return $this->upsertMultiple($tableExpression, $data, $types, $columnsToUpdate, $valuesToUpdate);
    }
    ...
    protected function insertMultiple($tableExpression, array $data, array $types = [])
    {
        ...
        return $this->executeStatement($sql, $params, $finalTypes);
    }

    protected function insertIgnoreMultiple($tableExpression, array $data, array $types = [])
    {
        $values = [];
        $params = [];
        ...
        $first = reset($data);
        $sql = 'INSERT IGNORE INTO ' . $tableExpression . ' (' . implode(', ', array_keys($first)) . ') VALUES '
            . implode(', ', $values);
     ...
     * @throws Exception
     */
    protected function upsertMultiple($tableExpression, array $data, array $types = [], array $columnsToUpdate = [], array $valuesToUpdate = [])
    {
        $first = reset($data);
        $columnsToUpdate = !empty($columnsToUpdate) ? $columnsToUpdate : array_keys($first);
        $updates = [];

        foreach ($columnsToUpdate as $i => $columnName) {
            if (empty($valuesToUpdate)) {
                $updates[] = $columnName . '=' . 'VALUES(' . $columnName . ')';
            } else {
                $updates[] = $columnName . '='. $valuesToUpdate[$i];
            }
        }
